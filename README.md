# GDManualAP
for my chud diggers </3




win con: collect all guns (including mods)
locations: certain kills with X weapon (accumulative), certain kills with X loadout (primary secondary perk class class gear)
what if i actually randomly generate the loadouts..... JROOOOOL
so its not like every loadout is specifically primary + secondary + perk + class + class gear(s), some could just be primary + secondary or something like that. but it has to have at least 2. also randomized kill count? idk
if possible make filler item name randomize between different queens will/kings decree announcement things

maybe add a thing to make like 2 kill locations per gun (e.g. 9 and 18)


kill ceiling: 25-30

options for # of accumulated kills
options for which classes should be in the pool
options for how many randomized loadouts there should be